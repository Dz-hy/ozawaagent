...
- [x] XSS fixture 回归门禁（2026-08-12）：`test/chat/markdown-xss.test.mjs` 纳入 `pnpm test:frontend`——真实 streamdown sanitize/harden 链端到端覆盖聊天/工作区链全向量、`data:` URL mimeType 白名单（`src/lib/chat/imageDataUrl.ts` 加固：非 image/* 不构造 data: URL、空 src 不渲染 img）、mermaid securityLevel strict 配置；详见 threat-model §9.5。
