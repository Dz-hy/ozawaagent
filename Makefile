...
DESKTOP_MACOS_APP_NAME ?= OzawaAgent
DESKTOP_MACOS_NOTARY_PROFILE ?= ozawaagent-notary
...
DESKTOP_RELEASE_TAURI_CONFIG_FLAGS ?= --config $(DESKTOP_RELEASE_TAURI_CONFIG) $(if $(OZAWAAGENT_TAURI_VERSION_CONFIG),--config $(OZAWAAGENT_TAURI_VERSION_CONFIG))
...
github-release-main: check-github-release-tag
	...
	git tag -a "$(RELEASE_TAG)" -m "OzawaAgent $(RELEASE_TAG)"
