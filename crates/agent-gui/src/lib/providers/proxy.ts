...
export const OZAWAAGENT_PROXY_TOKEN_HEADER = "x-ozawaagent-proxy-token";
export const OZAWAAGENT_UPSTREAM_ORIGIN_HEADER = "x-ozawaagent-upstream-origin";
export const OZAWAAGENT_UPSTREAM_USER_AGENT_HEADER = "x-ozawaagent-upstream-user-agent";
export const OZAWAAGENT_UPSTREAM_CONTENT_TYPE_HEADER = "x-ozawaagent-upstream-content-type";
// 布尔标记头：声明该请求经系统代理出网。代理地址/凭据只存于桌面 Rust 侧，
// 由本地反代按此头选择带代理的 client（x-ozawaagent-* 头不会转发给上游）。
export const OZAWAAGENT_USE_SYSTEM_PROXY_HEADER = "x-ozawaagent-use-system-proxy";
...
export async function prepareUpstreamProxyRequest(
  ...
  return {
    ...
    headers: {
      [OZAWAAGENT_UPSTREAM_ORIGIN_HEADER]: parsed.origin,
      [OZAWAAGENT_PROXY_TOKEN_HEADER]: proxyServerInfo.token,
      [OZAWAAGENT_USE_SYSTEM_PROXY_HEADER]: "1",
    },
  };
}
...
export async function prepareProxyRequest(
  ...
  return {
    ...
    headers: {
      ...
      [OZAWAAGENT_UPSTREAM_ORIGIN_HEADER]: upstreamOrigin,
      [OZAWAAGENT_PROXY_TOKEN_HEADER]: proxyServerInfo.token,
      ...(options?.useSystemProxy ? { [OZAWAAGENT_USE_SYSTEM_PROXY_HEADER]: "1" } : {}),
    },
  };
}
