    ...
    <div className="space-y-5">
      <div className="settings-section-heading-row flex items-center justify-between gap-4">
        <div className="settings-section-title-group flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold">{t("settings.commandPacksTitle")}</h3>
            <p className="text-xs text-muted-foreground">{t("settings.commandPacksDesc")}</p>
          </div>
        </div>
        <Button variant="outline" size="sm" onClick={() => void refresh()} disabled={loading}>
          <RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} />
          {t("settings.commandPacksRefresh")}
        </Button>
      </div>
      ...
      {snapshot && snapshot.conflicts.length > 0 ? (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-amber-600">
            ...
            {t("settings.commandPacksConflicts")} ({snapshot.conflicts.length})
          </div>
          <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
            {snapshot.conflicts.map((conflict) => (
              <li key={conflict.command_id}>
                ...
                {t("settings.commandPacksClaimedBy")} {conflict.sources.join(", ")}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-2xl border border-border/60 bg-card/40 p-4">
          <h4 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("settings.commandPacksDeclarative")} ({snapshot?.packs.length ?? 0})
          </h4>
          {snapshot && snapshot.packs.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              {t("settings.commandPacksNoPacks")} <span className="font-mono">command_packs/</span>.
            </p>
          ) : null}
            ...
            {(snapshot?.packs ?? []).map((pack) => (
              ...
              >
                ...
                <span className="shrink-0 text-[11px] text-muted-foreground">
                  {pack.command_ids.length} {t("settings.commandPacksCmds")}
                </span>
              </li>
            ))}
          </ul>
        ...
        <section className="rounded-2xl border border-border/60 bg-card/40 p-4">
          <h4 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("settings.commandPacksPythonPlugins")} ({snapshot?.plugins.length ?? 0})
          </h4>
          {snapshot && snapshot.plugins.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              {t("settings.commandPacksNoPlugins")}{" "}
              <span className="font-mono">command_plugins/</span>.
            </p>
          ) : null}
          <ul className="space-y-2">
            {(snapshot?.plugins ?? []).map((plugin) => (
              ...
              >
                ...
                <span className="shrink-0 text-[11px] text-muted-foreground">
                  {plugin.command_ids.length} {t("settings.commandPacksCmds")}
                </span>
              </li>
            ))}
          </ul>
      ...
      <p className="text-[11px] text-muted-foreground">
        {snapshot ? `${snapshot.loaded_command_count} ${t("settings.commandPacksAvailable")}` : ""}{" "}
        {t("settings.commandPacksFooter1")} <span className="font-mono">command_packs/</span>{" "}
        {t("settings.commandPacksFooter2")} <span className="font-mono">command_plugins/</span>{" "}
        {t("settings.commandPacksFooter3")}.
      </p>
    </div>
  );
}
