...
test("release version script writes GitHub env, outputs, and Tauri config", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "ozawaagent-version-"));
  try {
    ...
    assert.match(readFileSync(envPath, "utf8"), /^OZAWAAGENT_APP_VERSION=1\.2\.3$/m);
    assert.match(
      readFileSync(envPath, "utf8"),
      new RegExp(`^OZAWAAGENT_TAURI_VERSION_CONFIG=${escapeRegExp(tauriConfigPath)}$`, "m"),
    );
