...
pub(crate) fn history_message_content_hash(message: &Value) -> String {
    ...
    let role = object
        ...
        .and_then(Value::as_str)
        .unwrap_or_default();
    let mut parts = vec!["ozawaagent-history-ref-v1".to_string()];
    append_hash_part(&mut parts, role);

    if role == "user" {
        let display_text = object
            .and_then(|object| object.get("ozawaAgentDisplayContent"))
            .and_then(Value::as_str)
            .map(str::to_string)
            .unwrap_or_else(|| {
        ...
        let attachments = object
            .and_then(|object| object.get("ozawaAgentAttachments"))
