...
//! state changes and pushes debounced invalidation events to the local
//! webview (`workspace:activity`).
//!
//! The local webview (Tauri command `workspace_watch_set`) feeds the desired
//! watch set; each `set_desired` call replaces the set wholesale and
...
impl WorkspaceWatchService {
    ...
    pub fn set_desired(self: &Arc<Self>, source: WatchSource, workdirs: Vec<String>) {
        ...
        let desired: BTreeSet<String> = inner.local.clone();
