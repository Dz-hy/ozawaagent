...
function usage() {
  return [
    ...
    "",
    "Options:",
    "  --github-env <path>      Append OZAWAAGENT_* variables for later workflow steps.",
...
try {
  ...
  const metadata = parseReleaseVersion(
    options.releaseTag || process.env.OZAWAAGENT_RELEASE_TAG || process.env.RELEASE_TAG,
  );
  ...
  if (options.githubEnvPath) {
    const envLines = [
      `OZAWAAGENT_RELEASE_TAG=${metadata.releaseTag}`,
      `OZAWAAGENT_APP_VERSION=${metadata.appVersion}`,
      `OZAWAAGENT_IS_PRERELEASE=${metadata.isPrerelease}`,
    ];
    if (options.tauriConfigPath) {
      envLines.push(`OZAWAAGENT_TAURI_VERSION_CONFIG=${options.tauriConfigPath}`);
    }
  ...
  if (options.json) {
    ...
    const configSuffix = options.tauriConfigPath
      ...
      : "";
    console.log(
      `Prepared OzawaAgent ${metadata.releaseTag} (app version ${metadata.appVersion}, prerelease ${metadata.isPrerelease}).${configSuffix}`,
    );
  }
} catch (error) {
