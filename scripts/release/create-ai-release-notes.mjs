...
function fallbackNotes() {
  ...
  return `# OzawaAgent ${releaseVersion.releaseTag}\n\nRelease ${releaseVersion.releaseTag}.`;
}
...
function normalizeMarkdown(markdown) {
  ...
  if (!output.startsWith("#")) {
    output = `# OzawaAgent ${releaseVersion.releaseTag}\n\n${output}`;
  }
  return `${output.trim()}\n`;
}
...
function collectContext() {
  ...
  const repository = process.env.GITHUB_REPOSITORY?.trim() || "Dz-hy/ozawaagent";
...
function buildPrompt(context) {
  return [
    ...
    "- Start with exactly this H1: # OzawaAgent " + context.releaseTag,
